...
func RouteCreateHandler(event *RouteCreatedEvent, routeService *RouteService) (*FreightCalculatedEvent, error) {
...
func DeliveryStartHandler(event *DeliveryStartedEvent, routeService *RouteService, ch chan *DriverMovedEvent) error {
	...
	go func() {
		for _, direction := range route.Directions {
			ch <- NewDriverMovedEvent(route.ID, direction.Lat, direction.Lng)
