...
func NewRouteService(mongodb *mongo.Client, freightService *FreightService) *RouteService {
	return &RouteService{
		mongodb:        mongodb,
		freightService: freightService,
	}
...
func (rs *RouteService) CreateRoute(route *Route) (*Route, error) {
	...
	filter := bson.M{"id": route.ID}
	opts := options.Update().SetUpsert(true)
	_, err := rs.mongodb.Database("routes").Collection("routes").UpdateOne(nil, filter, update, opts)
	if err != nil {
		return nil, err
	}
	return route, err
...
func (rs *RouteService) GetRoute(id string) (Route, error) {
	var route Route
	filter := bson.M{"id": id}
	err := rs.mongodb.Database("routes").Collection("routes").FindOne(nil, filter).Decode(&route)
	if err != nil {
		return Route{}, err
	}
	return route, nil
