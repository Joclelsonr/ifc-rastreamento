      ...
      async (data: { route_id: string; lat: number; lng: number }) => {
        if (!map.hasRoute(data.route_id)) {
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/routes/${data.route_id}`
          );
          const route = await response.json();
          map.addRouteWithIcons({
            routeId: data.route_id,
            startMarkerOptions: {
              position: route.direction.routes[0].legs[0].start_location,
            },
            endMarkerOptions: {
              position: route.direction.routes[0].legs[0].end_location,
            },
            carMarkerOptions: {
              position: route.direction.routes[0].legs[0].start_location,
            },
